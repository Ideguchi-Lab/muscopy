...
def numpy_parser(dir_path: str) -> list[str]:
    ...
    """
    return [
        str(file) for file in pathlib.Path(dir_path).iterdir() if file.is_file() and file.suffix == ".npy"
    ]
...
def png_parser(dir_path: str) -> list[str]:
    ...
    """
    return [
        str(file) for file in pathlib.Path(dir_path).iterdir() if file.is_file() and file.suffix == ".png"
    ]
...
def tiff_parser(dir_path: str) -> list[str]:
    ...
    """
    return [
        str(file) for file in pathlib.Path(dir_path).iterdir() if file.is_file() and file.suffix == ".tiff"
    ]
