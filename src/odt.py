...
class ODTParameters(QPIParameters):
    def __init__(self, wavelength, NA, img_shape, img_center, pixelsize, offaxis_center, n_sol):
...
class ODTSynthesizer(Synthesizer):
    def set_parameters(self, wavelength, NA, img_shape, img_center, pixelsize, offaxis_center, n_sol):
        self.params = ODTParameters(wavelength, NA, img_shape, img_center, pixelsize, offaxis_center, n_sol)
        self.params.calc_params()

    def ODT_synthesize(self, background_region=[[(0, 50), (0, 50)]]):
        # calculate Z dimension
        z_plus = self.params.ki_mag - np.sqrt(self.params.ki_mag**2 - (self.params.aperturesize // 2) ** 2)
        z_minus = -self.params.ki_mag - np.sqrt(self.params.ki_mag**2 - (self.params.aperturesize // 2) ** 2)
        z_length = int(np.ceil(z_plus - z_minus))
        z_center = round(-z_minus)

        memory_per_picture = (2 * self.params.aperturesize + 1) ** 2 * z_length * 16 * 1e-9
        print("memory per picture: {} GB".format(memory_per_picture))

        synthesized_fft = xp.zeros(
            (2 * self.params.aperturesize + 1, 2 * self.params.aperturesize + 1, z_length), dtype=xp.complex128
        )
        synthesized_center = (self.params.aperturesize, self.params.aperturesize, z_center)
        synthesized_weight = xp.ones(
            (2 * self.params.aperturesize + 1, 2 * self.params.aperturesize + 1, z_length), dtype=xp.int64
        )
        ...
        for i in tqdm(range(len(pic_path_list))):
            ...
            array = xp.array(Image.open(os.path.join(self.target_path, filename))).reshape(self.params.img_shape)
            ref_array = xp.array(Image.open(os.path.join(self.ref_path, ref_filename))).reshape(self.params.img_shape)

            fft = xp.fft.fftshift(xp.fft.fftn(array))
            ref_fft = xp.fft.fftshift(xp.fft.fftn(ref_array))

            oblique_center = self.oblique_centers[i]
            disk = make_disk(self.params.off_axis, self.params.aperturesize / 2, self.params.img_shape)
            ...
            kz_i = xp.sqrt(self.params.ki_mag**2 - oblique_center[0] ** 2 - oblique_center[1] ** 2)
            fft_3d, array_weight = map_to_3d(
                fft_divided, synthesized_fft.shape, oblique_center, synthesized_center, self.params.aperturesize, kz_i
            )
...
def map_to_3d(array, shape, oblique_center, synthesized_center, aperturesize, kz):
    ...
    center_sphere = (synthesized_center[0] + oblique_center[0], synthesized_center[1] + oblique_center[1], kz)
...
def make_sphere_surface(center, radius, array_shape):
    ...
    xx, yy, zz = xp.meshgrid(
        xp.arange(array_shape[0]), xp.arange(array_shape[1]), xp.arange(array_shape[2]), indexing="xy"
    )
